...
# Map Ozmo/Deebot raw statuses to HA VacuumActivity
ACTIVITY_MAP = {
...
class EcovacsDeebotVacuum(StateVacuumEntity):
    ...
    def __init__(self, device, config):
        ...
        # Bitmask from __init__.py (VacuumEntityFeature)
        self._supported_features = config.get(CONF_SUPPORTED_FEATURES, 0)

        # Ensure STATE is present for modern vacuum UI behavior
        self._supported_features |= VacuumEntityFeature.STATE

        _LOGGER.debug(
            "Vacuum initialized: %s with features: %s",
            self.name,
    ...
    def on_error(self, error):
        ...
        self.hass.bus.fire("ecovacs_error", {"entity_id": self.entity_id, "error": error})
    ...
    def activity(self) -> VacuumActivity:
        ...
        return ACTIVITY_MAP.get(self.device.vacuum_status, VacuumActivity.ERROR)

    @property
    def state(self) -> str:
        """Legacy vacuum state for StateVacuumEntity compatibility."""
        return self.activity.value

    @property
    def status(self):
        """Return the raw status of the vacuum cleaner."""
    ...
    def battery_level(self):
        """Return the battery level of the vacuum cleaner (0-100)."""
    ...
    def send_command(self, command, params=None, **kwargs):
        ...
        if command == "set_water_level":
            return self.set_water_level(params.get("level"))

        self.device.run(VacBotCommand(command, params))

    def clean_map(self, map_position):
        ...
        if not map_position:
            ...
            self.device.run(
                SpotArea(map_position=map_position, speed=self.fan_speed, action="start")
            )
    ...
    def extra_state_attributes(self):
        ...
        # Keep ATTR_STATUS for any legacy cards/automations expecting it
        data[ATTR_STATUS] = self.activity.value
